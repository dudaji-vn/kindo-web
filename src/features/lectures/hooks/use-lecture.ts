import { useGetCourses, type CourseRecord } from '@/features/courses/hooks/use-get-courses';
import { useGetLectures, type LectureRecord } from '@/features/lectures/hooks/use-get-lectures';
import { useGetLessons, type LessonRecord } from '@/features/lessons/hooks/use-get-lessons';
...
export const useLecture = () => {
  const { data: courses, isLoading: isCoursesLoading, error: coursesError } = useGetCourses();
  const { data: lessons, isLoading: isLessonsLoading, error: lessonsError } = useGetLessons();
  const { data: lectures, isLoading: isLecturesLoading, error: lecturesError } = useGetLectures();
    ...
    return courses
      .map((course) => {
        ...
        const lessonsWithLectures: LessonWithLectures[] = lessonsForCourse
          .map((lesson) => ({
            lesson,
            lectures: lectures.filter((lec) => lec.course_id === course.id && lec.lesson_id === lesson.id),
          }))
